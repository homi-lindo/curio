...
If sync is enabled, Curió sends tasks, notes, tombstones, and sync metadata to
the server URL configured by the user. The optional sync server can be
self-hosted with Docker, started as an in-app Windows local server, or replaced by
another user-controlled endpoint that implements the same API.
